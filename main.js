...
const chatList = {
    ...
    // userCount2：名前
    8: {text: '', continue: true, option: 'normal'},
    9: {text: '今日の体調はいかがですか？', continue: false, option: 'normal'},
    // userCount3：体調
    10: {text: ['そうですか！', 'なるほどです！', '承知致しました！'], continue: true, option: 'random'},
    11: {text: 'ここで問題です！', continue: true, option: 'normal'},
    12: {text: {title: 'Q2', question: 'このChatbotがモチーフにしているアプリは何でしょう？', choices: ['Twitter', 'LINE', 'Discord'], answer: '1'}, continue: false, option: 'choices'},
    // userCount4：Q2
    13: {text: {qTrue: '', qFalse:'残念！正解は「LINE」でした。'}, continue: true, option: 'normal'},
    14: {text: '続いての問題です！', continue: true, option: 'normal'},
    15: {text: {title: 'Q3', question: 'これまでにいくつ質問したでしょうか？', choices: ['２回', '３回', '４回'], answer: '2'}, continue: false, option: 'choices'},
    // userCount5：Q3
    16: {text: {qTrue: '', qFalse:'残念！正解は「４回」でした。'}, continue: true, option: 'normal'},
    17: {text: '', continue: true, option: 'normal'},
    18: {text: {title: '満足度調査', question: 'この「AI Chatbot」の満足度を5段階で教えてください（数字が大きいほど満足度が高いものとします。）。', choices: ['5', '4', '3', '2', '1']}, continue: false, option: 'choices'},
    // userCount6：満足度
    19: {text: 'ありがとうございます。最後に、ご感想をお聞かせください。', continue: false, option: 'normal'},
    // userCount7：感想
    20: {text: '', continue: true, option: 'normal'},
    21: {text: '以下、「AI Chatbot」は自動返答機能に切り替わります。ご了承ください。', continue: false, option: 'normal'},
};
...
function textSpecial() {
    chatList[8].text = `こんにちは！${userData[1]}さん！`;
    chatList[13].text.qTrue = `正解！${userData[1]}さん、すごいですね！`;
    chatList[16].text.qTrue = `正解！${userData[1]}さん、お見事です！`;
    chatList[17].text = `${userData[1]}さん、ありがとうございました。本日はここで終了とさせていただきます。`; 
    chatList[20].text = `${userData[1]}さんの満足度は「${userData[5]}」，ご感想は「${userData[6]}」ですね！ありがとうございました。`;
}
    ...
    setTimeout( ()=> {
        ...
        if (chatList[robotCount].option === 'choices') {
            ...
            switch(chatList[robotCount].option) {
                case 'normal':
                    if (chatList[robotCount].text.qTrue) {
                        ...
                        if(chatList[robotCount].link) {
                            div.innerHTML = `<a href= "${chatList[robotCount].text[nextTextOption]} " target="_blank" onclick= "chatbotLinkClick()">${chatList[robotCount].text[nextTextOption]}</a>`;
                        } else {
                        ...
                        // 答えのない質問（次にサポートあり）
                        if(chatList[robotCount].link) {
                            div.innerHTML = `<a href= "${String(chatList[robotCount].text[nextTextOption])}" target="_blank" onclick= "chatbotLinkClick()">${String(chatList[robotCount].text[nextTextOption])}</a>`;
                        } else {
                        ...
                        // 通常
                        if(chatList[robotCount].link) {
                            div.innerHTML = `<a href= "${chatList[robotCount].text}" target="_blank" onclick= "chatbotLinkClick()">${chatList[robotCount].text}</a>`;
                        } else {
                ...
                case 'random':
                    if(chatList[robotCount].link) {
                        div.innerHTML = `<a href= "${chatList[robotCount].text[Math.floor(Math.random() * chatList[robotCount].text.length)]}" target="_blank" onclick= "chatbotLinkClick()">${chatList[robotCount].text[Math.floor(Math.random() * chatList[robotCount].text.length)]}</a>`;
                    } else {
